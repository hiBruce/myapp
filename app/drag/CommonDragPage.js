...
const childrenWidth = deviceWidth / 10;
...
export default class CommonDragPage extends Component {
    ...
    renderSelectedItemView = (item, index) => {
        const clearIcon = this.state.isEditState ?
            <TouchableOpacity
            onPress={() => {this.onSelectedClickItem(item, index)}}
                style={styles.selected_item_icon}>
                <Image source={require('./../../imgs/jianhao.png')} />
            </TouchableOpacity>
            : undefined
        ...
        return (
            <View style={styles.selected_container}>
                ...
                <Image style={{ width: 16, height: 16, resizeMode: 'contain' }} source={require('./../../imgs/jianhao.png')} />
    ...
    renderUnSelectedItemView = (item, index) => {
        const addIcon = this.state.isEditState ?
            <TouchableOpacity
            onPress={(item, index) => {this.onUnSelectedClickItem(item, index)}}
                style={styles.selected_item_icon}>
                <Image style={{ width: 16, height: 16, resizeMode: 'contain' }} source={require('./../../imgs/add.png')} />
    ...
    onSelectedClickItem = (item, index) => {
        alert(item.text + index)
        // delete, data 是最新的数据
        if (this.state.isEditState) {
            this.setState({
                selectedItems: [...this.state.selectedItems].filter((wItem, windex) => windex !== index),
                unselectedItems: [item, ...this.state.unselectedItems]
            })
        }
    }

    onUnSelectedClickItem = (item, index) => {
        if (this.state.isEditState) {
            this.setState({
                selectedItems: [...this.state.selectedItems, item],
                unselectedItems: [...this.state.unselectedItems].filter((wItem, windex) => windex !== index)
            })
        }
    }
